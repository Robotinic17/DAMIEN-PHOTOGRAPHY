...
faqArray.forEach((data) => {
  ...
  accordionItem.innerHTML = `
    ...
    <div>
      <p style="font-size:15px; color:rgba(233, 233, 233, 0.79); margin-bottom:20px; " >${
        data.title
      }</p>
    <p style="font-size:12px; color:rgba(128, 128, 128, 0.79);" >${
      data.desc
    }</p>
    </div>
    <div><button style=" background:none; border:0.5px solid rgba(128, 128, 128, 0.38); padding:10px 12px; border-radius: 25px; color:rgba(128, 128, 128, 0.85);" >${
      activeId == data.id ? ">" : "<"
    }</button></div>
    </div>
  `;
